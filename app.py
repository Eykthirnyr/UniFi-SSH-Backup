...
def load_config():
    ...
    if 'retry_delay' not in cfg:
        cfg['retry_delay'] = 30
    if 'report_fail_only' not in cfg:
        cfg['report_fail_only'] = False
    return cfg
...
def update_config_from_request(form):
    ...
    for key in [
        'ip', 'ssh_user', 'ssh_pass', 'backup_time',
        'smtp_host', 'smtp_user', 'smtp_pass', 'smtp_to',
        'client_name', 'timezone'
    ]:
        config[key] = form.get(key, '')
    config['ssh_port'] = int(form.get('ssh_port', 22))
    config['smtp_port'] = int(form.get('smtp_port', 587))
    config['retry_delay'] = int(form.get('retry_delay', 30))
    config['send_report'] = True if form.get('send_report') == 'on' else False
    config['report_fail_only'] = True if form.get('report_fail_only') == 'on' else False
    if config['report_fail_only']:
        config['send_report'] = False
    elif config['send_report']:
        config['report_fail_only'] = False
...
def find_today_backup(sftp):
    ...
    The function also returns the modification time of the chosen file.
    """
    ...
    if candidates:
        ...
        mtime = sftp.stat(f"{remote_dir}/{fname}").st_mtime
        return fname, False, datetime.datetime.fromtimestamp(mtime, tz)
    # Fallback to newest .unf file if today's not found
    if not files:
        return None, False, None
    def get_mtime(f):
        return sftp.stat(f"{remote_dir}/{f}").st_mtime
    latest = max(files, key=get_mtime)
    mtime = get_mtime(latest)
    log(f"Today's backup not found, using latest file {latest}", ssh=True)
    return latest, True, datetime.datetime.fromtimestamp(mtime, tz)
...
def run_backup():
    ...
    try:
        ...
        filename, fallback, mod_time = find_today_backup(sftp)
        if not filename:
        ...
        if fallback:
            fallback_str = mod_time.strftime('%d/%m/%Y %H:%M:%S')
            msg = f'Backup successful (latest from {fallback_str})'
        ...
        if fallback:
            h['message'] = f"Could not find today's backup, used latest day instead {fallback_str}"
        else:
            h.pop('message', None)
        ...
        if RETRY_JOB_ID:
            try:
                ...
                pass
            RETRY_JOB_ID = None
        if config.get('send_report') and not config.get('report_fail_only'):
            send_mail(
...
def settings():
    ...
    if request.method == 'POST':
        old_port = config.get('web_port')
        update_config_from_request(request.form)
        schedule_job()
        flash('Settings saved')
        if config.get('web_port') != old_port:
            return render_template('restart.html', port=config.get('web_port'))
        return redirect(url_for('settings'))
    return render_template('settings.html', config=config, timezones=TIMEZONES)


@app.route('/do_restart')
def do_restart():
    """Restart the application process to apply new settings."""
    log('Restarting application to apply configuration change')
    save_config(config)
    try:
