    ...
    try:
        ...
        if config.get('retry_on_fail'):
            ...
            if next_time.date() == now.date():
                ...
                log(f"Scheduled retry at {next_time.isoformat()}")
            else:
                log('Next day reached, retry aborted')
...
def settings():
    ...
    if request.method == 'POST':
        for key in ['ip', 'ssh_port', 'ssh_user', 'ssh_pass', 'backup_time', 'smtp_host', 'smtp_port', 'smtp_user', 'smtp_pass', 'smtp_to', 'smtp_from', 'client_name', 'timezone', 'retry_delay']:
        ...
        config['retry_on_fail'] = True if request.form.get('retry_on_fail') == 'on' else False
